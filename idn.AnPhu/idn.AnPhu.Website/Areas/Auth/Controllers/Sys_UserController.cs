...
namespace idn.AnPhu.Website.Areas.Auth.Controllers
{
    public class Sys_UserController : AdministratorController// : Controller
    {
        ...
        public ActionResult Index(string usercode = "", string fullname = "", string createdtimefrom = "", string createdtimeto = "", string email = "", string flagactive = "", string init = "init", int page = 0)
        {
            var startCount = "0";
            ...
            if (init != "init")
            {
                ...
                var sex = "";
                var issysadmin = "";
                pageInfo = Sys_UserManager.Search(usercode, fullname, phoneno, email, sex, flagactive, issysadmin, page, PageSizeAdminConfig);
        ...
        {
            var resultEntry = new JsonResultEntry() { Success = false };
            var objSys_User = new Sys_User() { UserCode = usercode };
