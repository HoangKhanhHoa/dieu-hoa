...
namespace idn.AnPhu.Biz.Services
{
    public class Sys_UserManager : DataManagerBase<Sys_User>
    {
        ...
        public PageInfo<Sys_User> Search(string userCode, string fullName, string phoneNo, string email, string sex, string flagActive, string isSysAdmin, int pageIndex, int pageCount)
        {
            int totalItems = 0;
            var pageIndexCur = pageIndex * pageCount;
            var page = new PageInfo<Sys_User>(pageIndex, pageCount);
            var list = Sys_UserProvider.Search(userCode, fullName, phoneNo, email, sex, flagActive, isSysAdmin, pageIndexCur, pageCount, ref totalItems);
            page.DataList = list;
